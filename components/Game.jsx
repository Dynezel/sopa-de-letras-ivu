...
import { WORDS, GROUPS, GRID_SIZE, ADMIN_PASSWORD } from "../config";
import "./Game.css";
...
export function Game({ myGroup, onLeaveGroup }) {
  ...
  // Calcular colores de celdas encontradas (primer grupo en encontrar)
    ...
    >
      ...
      <header className="game-header">
        <div className="game-header__left">
          <div
            className="game-header__group-badge"
            style={{
              background: myGroup.color + "18",
              border: `1px solid ${myGroup.color}55`,
            }}
          >
          ...
          <button className="game-header__change-btn" onClick={onLeaveGroup}>
            cambiar
          </button>
        </div>
        ...
        <div className="game-header__right">
          ...
          <button className="game-header__admin-btn" onClick={() => setShowPwDialog(true)}>
            ⚙
          </button>
        </div>
      </header>
      ...
      <div className="game-stats">
        <div className="game-stats__progress">
          ...
          <div className="game-stats__progress-track">
            <div
              ...
              style={{
                width: `${(myFound.length / WORDS.length) * 100}%`,
                background: myGroup.color,
                boxShadow: `0 0 8px ${myGroup.color}66`,
              }}
            />
          </div>
        </div>
        ...
        >
          ...
          <span
            className="game-stats__firsts-count"
            style={{ color: myFirstFinds > 0 ? myGroup.color : "#222" }}
          >
            {myFirstFinds}
          </span>
        </div>
      ...
      {allFound && started && (
        ...
        >
          ...
          <span className="game-completed__title" style={{ color: myGroup.color }}>
            ¡COMPLETASTE TODAS!
          </span>
          {myFirstFinds > 0 && (
      ...
      {flash && (
        <div
          className={`game-flash ${flash.isFirst ? "game-flash--first" : "game-flash--late"}`}
          style={
            flash.isFirst
        ...
        <div className="pw-overlay">
          <div className="pw-dialog">
            ...
            <div className="pw-dialog__buttons">
              <button
                className="pw-dialog__cancel"
                onClick={() => { setShowPwDialog(false); setPwInput(""); setPwError(false); }}
              >
                Cancelar
              </button>
              <button className="pw-dialog__submit" onClick={submitPw}>
                Entrar
              </button>
            </div>
          </div>
